  ...
  if (!isStudyDay) {
    return (
      <Card className="p-6 text-center">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-3 bg-muted">
          <CheckCircle2 className="h-6 w-6 text-primary" />
        </div>
        <h3 className="font-semibold text-foreground mb-1">Dia de descanso!</h3>
  ...
  if (plan.length === 0) {
    return (
      <Card className="p-6 text-center">
        ...
        <h3 className="font-semibold text-foreground mb-1">Adicione disciplinas ao seu plano</h3>
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h3 className="font-bold text-foreground text-sm">Plano de Hoje</h3>
            </div>
          ...
          <div className="text-right">
            <div className="flex items-center gap-1 justify-end">
              ...
              <span className="text-sm font-bold text-foreground">{Math.round(totalMinutes / 60 * 10) / 10}h</span>
            </div>
            <p className="text-xs text-muted-foreground">{plan.length} sessões</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 space-y-2">
        {plan.map((item, idx) => (
          <div
            ...
            className="flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 cursor-pointer group bg-muted border border-border hover:border-primary/30"
            onClick={() => navigate('/registrar', { state: { suggestedSubjectId: item.subjectId, suggestedTopicId: item.topicId } })}
          >
            {/* Color dot */}
            <div
              className="w-2.5 h-2.5 rounded-full flex-shrink-0"
              style={{ background: item.subjectColor || 'var(--gradient-primary)' }}
            />

            {/* Content */}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">{item.topicName}</p>
              <p className="text-xs truncate text-muted-foreground">{item.subjectName}</p>
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <Badge
                className="text-xs px-2 py-0.5 bg-secondary text-muted-foreground border-none"
              >
                {item.minutes}min
              </Badge>
              <ArrowRight
                className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 transition-opacity text-primary"
              />
            </div>
          </div>
        ))}
      ...
      <div className="px-5 py-3 border-t border-border">
        <Button
          className="w-full h-9 text-sm font-medium rounded-xl text-white"
